...
(function () {
  ...
  const KIND_COLOR = { DREAM:'#4fd6e0', COSMOGONY:'#e8b13a', MYTH:'#e9923f', SHADOW:'#9a6cff', ABOUT:'#9bb08a' };

  const state = {
    coll: 'garden', name: '', id: '', idTouched: false,
    kind: 'DREAM', color: '#4fd6e0', flower: '✿', h: 176,
  ...
  function buildEntry(forExport) {
    ...
    return A.sign({ id: state.id, name: state.name, color: state.color, h: state.h,
      desc: state.desc, text: state.text, gif: src || undefined });
  }

  /* ---------- live preview ---------- */
  function renderPreview() {
    const c = state.color, stage = $('pv-stage'), page = $('pv-page');
    if (state.coll === 'garden') {
      stage.innerHTML = `<div class="bed"><div class="bloom" style="--c:${c};--stem:90px">`
        + `<div class="flower">${state.flower}</div></div></div>`;
    } else {
      stage.innerHTML = `<div class="shelf"><div class="book" style="--c:${c};--h:${state.h}px">`
        + `<div class="spine">${state.name || 'UNTITLED'}</div></div></div>`;
    }
    let html = '';
    const src = gifSrc();
  ...
  function setColl(coll) {
    ...
    if (coll === 'library' && state.color === KIND_COLOR[state.kind]) setColor('#cf8a4a');
    renderPreview();
  }
  ...
  $('seg-h').addEventListener('click', e => { const b = e.target.closest('button'); if (!b) return;
    state.h = +b.dataset.h; [...$('seg-h').children].forEach(x => x.classList.toggle('on', x === b)); renderPreview(); });
  ...
  $('color-swatches').addEventListener('click', e => { const s = e.target.closest('.swatch'); if (s) setColor(s.dataset.c); });
  $('f-color').addEventListener('input', e => setColor(e.target.value));
  ...
  $('f-kind').addEventListener('change', e => { state.kind = e.target.value; setColor(KIND_COLOR[state.kind] || state.color); });
  ...
  $('btn-clear').addEventListener('click', () => { if (confirm('Remove all locally-saved preview pages?')) { writeDrafts([]); renderDrafts(); } });
  $('drafts-list').addEventListener('click', e => { const a = e.target.closest('a.del'); if (!a) return;
    writeDrafts(readDrafts().filter(x => x.id !== a.dataset.id)); renderDrafts(); });

  /* ---------- init ---------- */
  setColl('garden'); setColor('#4fd6e0'); renderDrafts(); renderPreview();
})();
